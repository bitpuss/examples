...
layout: post
title: Go Slice 示例
permalink: go-basic/golang-slice-example.html
class: golang
categories: ['go-basic']
---

切片是 Go 语言的关键类型之一，它提供了比数组更多的功能。
...
`cap` 是 `slice` 的最大容量，`append` 函数添加元素，如果超过原始 `slice` 的容量，会重新分配底层数组。
...
指向同一底层数组的 `slice` 之间 `copy` 时，允许存在重叠。`copy` 数组时，受限于 `src` 和 `dst` 数组的长度最小值。
